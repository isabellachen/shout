<?php
/**
 * shout_theme functions and definitions
 *
...
if ( ! defined( 'SHOUT_THEME_VERSION' ) ) {
	// Replace the version number of the theme on each release.
	define( 'SHOUT_THEME_VERSION', '1.0.0' );
}

if ( ! function_exists( 'shout_theme_setup' ) ) :
	/**
	 ...
	 */
	function shout_theme_setup() {
		/*
		 * Make theme available for translation.
		 * Translations can be filed in the /languages/ directory.
		 * If you're building a theme based on shout_theme, use a find and replace
		 * to change 'shout_theme' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'shout_theme', get_template_directory() . '/languages' );
    ...
    function end_el( &$output, $item, $depth = 0, $args = array() ) {
      $output .= "\n";
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'shout_theme_custom_background_args',
				array(
...
add_action( 'after_setup_theme', 'shout_theme_setup' );
 ...
 */
function shout_theme_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'shout_theme_content_width', 640 );
}
add_action( 'after_setup_theme', 'shout_theme_content_width', 0 );
 ...
 */
function shout_theme_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'shout_theme' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'shout_theme' ),
...
add_action( 'widgets_init', 'shout_theme_widgets_init' );

/**
 * Enqueue scripts and styles.
 */

function shout_theme_scripts() {
  ...
  wp_enqueue_script( 'bootstrap' );
	...
	wp_enqueue_style( 'shout_theme-style', get_stylesheet_uri(), array(), SHOUT_THEME_VERSION );
	wp_style_add_data( 'shout_theme-style', 'rtl', 'replace' );

	wp_enqueue_style( 'din-font', "https://use.typekit.net/vmm5oll.css", array(), null );

	wp_enqueue_script( 'shout_theme-navigation', get_template_directory_uri() . '/js/navigation.js', array('jquery'), SHOUT_THEME_VERSION, true );
...
add_action( 'wp_enqueue_scripts', 'shout_theme_scripts' );
